# Linz Open Data hackathon review

Start with the [final dataset verdicts](final-verdicts/). They provide one folder
per reviewed dataset or dataset bundle, preserve Rainer's original verdict, and
add the final decision, preparation requirements, later endpoint checks, and open
publisher defects.

Supporting material:

- [Consolidated usability report](2026-07-13-linz-open-data-hackathon-usability.md)
- [Rainer's hands-on source reviews](2026-06-13-reviews-rainer/)
- [Benjamin's full 820-record catalog review](2026-06-13-reviews-benjamin/)
- [Superseded title/description candidate pass](2026-06-13-linz-datasets-candidates.md)
